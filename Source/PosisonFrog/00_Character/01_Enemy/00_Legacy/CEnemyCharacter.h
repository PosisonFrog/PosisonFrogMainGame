...
private:
	...
	float ChaseStartDistance = 800.0f; // 이 이내 + LoS → Alert
	
	UPROPERTY(EditAnywhere, Category = "Enemy|Sense")
	float ChaseStopDistance = 1200.0f; // 이 이상 or LoS 장시간 끊기면 추격 종료

	UPROPERTY(EditAnywhere, Category = "Enemy|Sense")
	float EasyChaseDistance = 200.0f;  // 매우 가까우면 LoS 생략

	UPROPERTY(EditAnywhere, Category = "Enemy|Sense")
	float LoseSightGrace = 1.0f;	   // LoS 끊겨도 이 시간 동안 추격 유지
	float LoseSightAcc = 0.0f;
