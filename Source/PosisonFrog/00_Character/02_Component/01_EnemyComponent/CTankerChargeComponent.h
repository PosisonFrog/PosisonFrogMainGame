...
// 상태 변경/종료 브로드캐스트
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnChargeStateChanged, EChargeState, NewState, EChargeState, PrevState);
...
public:
    UPROPERTY(BlueprintAssignable) FOnChargeStateChanged OnChargeStateChanged;
    UPROPERTY(BlueprintAssignable) FOnChargeFinished    OnChargeFinished;
...
private:
    ...
    // 충돌/피해 판정
    void ChargeTraceAndHit(float DeltaSeconds);
    bool SweepHitAhead(struct FHitResult& OutHit, float Distance) const;

    // 보조
    bool  IsValidTarget() const;
    float DistToTarget2D() const;
    void  FaceTowards(const FVector& To, float DeltaSeconds);
...
private:
    // ─ Runtime ─
    UPROPERTY(Transient) EChargeState State = EChargeState::Idle;
    ...
    // Charging
    FVector ChargeDir2D = FVector::ForwardVector;
    ...
    int32   LastSideSign = +1; // +1=Right, -1=Left
