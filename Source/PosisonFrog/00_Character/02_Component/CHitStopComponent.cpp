...
UCHitStopComponent::UCHitStopComponent()
{
	PrimaryComponentTick.bCanEverTick = false;
}

void UCHitStopComponent::BeginPlay()
{
	Super::BeginPlay();
}

void UCHitStopComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
bool UCHitStopComponent::StartHitStop(AActor* TargetActor, float Duration, float TimeScale)
{
	...
	TimerDelegate.BindUObject(this, &UCHitStopComponent::EndHitStopForActor, TargetActor);
...
bool UCHitStopComponent::IsActorInHitStop(AActor* TargetActor) const
{
	if (!IsValid(TargetActor))
		return false;
...
void UCHitStopComponent::StartMultipleHitStop(const TArray<AActor*>& TargetActors, float Duration, float TimeScale)
{
...
void UCHitStopComponent::StartPlayerAndEnemyHitStop(AActor* Player, AActor* HitEnemy, float Duration, float TimeScale)
{
	if (bShowDebugMessages)
		CLog::Log(TEXT("[HitStop] 플레이어와 적 히트 스탑 트리거됨"));

	int32 AppliedCount = 0;

	if (IsValid(Player))
	{
		if (StartHitStop(Player, Duration, TimeScale))
			AppliedCount++;
	}

	if (IsValid(HitEnemy))
	{
		if (StartHitStop(HitEnemy, Duration, TimeScale))
			AppliedCount++;
	}
...
void UCHitStopComponent::EndHitStop()
{
...
void UCHitStopComponent::EndHitStopForActor(AActor* TargetActor)
{
	if (!IsValid(TargetActor))
		return;
	...
	if (bShowDebugMessages)
		CLog::Log(FString::Printf(TEXT("[HitStop] %s의 커스텀 히트 스탑 종료 (컴포넌트 %d개)"), *TargetActor->GetName(), Components.Num()));
}
